...
class menus {
  ...
  def sixqueries(user:String): Unit ={
    ...
    println(s" ${Console.RED}[4]${Console.RESET} Past 10 Years Movies Rating Types Produced")
    println(s" ${Console.RED}[5]${Console.RESET} Leonardo DiCaprio Top Movies In Past 10 years")
    println(s" ${Console.RED}[6]${Console.RESET} Most Popular Movies and Shows In The Last 10 Years")
    ...
    while(isTrue) {
      input match {
        ...
        case "3" => println("clearscreen")
          ...
          sixqueries(user)
        case "4" => println("clearscreen")
          println("Past 10 Years Movies Rating Types Produced")
          sparkData.spark.sql("SELECT content_rating as Rating_Types, count(title) as total FROM movies GROUP BY Rating_Types ORDER BY total DESC").show(20,false)
          println(s"Press ${Console.GREEN}Enter${Console.RESET} to go back")
          val wait = readLine()
          sixqueries(user)
        case "5" => println("clearscreen")
          println("Leonardo DiCaprio Top Movies In Past 10 Years")
          ...
          sixqueries(user)
        case "6" => println("clearscreen")
          println("Most Popular Movies And Shows In The Last 10 Years")
